...
def grid_search_lr(model_ = create_1layer_model):   
    ...
    np.save(os.path.join('../array_and_figures','grid_kf=5_epo=40_b1=0.91_b2=0.999_btch=100_AmsGrad_lr_ro_CONV'),results)
...
def grid_search_beta(model_ = create_1layer_model):   
    ...
    for beta1 in beta1_list:
      ...
      for beta2 in beta2_list:
        ...
        results['val_loss'].append(val_loss_kfold)
        results['val_acc'].append(val_acc_kfold)
    np.save(os.path.join('../array_and_figures','grid_kf=5_epo=40_lr=1e-3_btch=100_Adam_beta1_beta2_ro_convex'),results)
